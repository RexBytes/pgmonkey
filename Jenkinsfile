pipeline {
    agent any
    environment {
        PIP_NO_CACHE_DIR = "off" // Disables pip caching to ensure fresh installs
    }
    stages {  // Add the "stages" section as required
        stage('Matrix Builds') {  // Grouping matrix in a single stage
            matrix {
                axes {
                    axis {
                        name 'PYTHON_VERSION'
                        values '3.12.5', '3.12.4', '3.12.3', '3.11.9', '3.10.14', '3.9.19', '3.8.19'
                    }
                    axis {
